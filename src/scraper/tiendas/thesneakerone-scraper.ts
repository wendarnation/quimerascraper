// src/scraper/tiendas/thesneakerone-scraper.ts
...
@Injectable()
export class TheSneakerOneScraper extends BaseTiendaScraper {
  ...
  /**
   * Método principal para raspar zapatillas de TheSneakerOne
   */
  async scrapeZapatillas(): Promise<ZapatillaScraped[]> {
    try {
      await this.initBrowser(this.getRandomUserAgent());

      // URL principal de zapatillas basada en la imagen 1 y 2
      const url = 'https://thesneakerone.com/zapatillas-sneaker';
      ...
      // Basado en la imagen 2, los productos están en elementos con clase "product_item"
      const productListSelector = '.product_item, .products li, .js-product-list, div[class*="product-miniature"]';
  ...
  private async extractProductUrls(selector: string): Promise<string[]> {
    ...
    // Basado en la imagen 2, buscar enlaces de productos en la página
    const productUrls = await this.page.evaluate((selector) => {
      ...
      const getVisibleUrls = () => {
        ...
        const productLinks = Array.from(
          document.querySelectorAll(`${selector} a.thumbnail`)
        );

        // Si no encuentra enlaces con ese selector, probar con una selección más general
        if (productLinks.length === 0) {
          // Intentar con otros selectores basados en las imágenes proporcionadas
          ...
          for (const altSelector of alternativeSelectors) {
            const links = Array.from(document.querySelectorAll(altSelector));
            if (links.length > 0) {
              return links
                .filter((element) => {
                  ...
                  return (
                    ...
                    element.href &&
                    element.href.includes('thesneakerone.com')
                  );
                })
  ...
  private async scrapeZapatillaDetail(url: string): Promise<ZapatillaScraped> {
    ...
    try {
      // Basado en las imágenes 3-6, estos son selectores para elementos clave del producto
      // Esperar a que cargue alguno de estos elementos
      await Promise.race([
        this.page.waitForSelector('.h1-main.productpage_title', { timeout: 10000 }),
        this.page.waitForSelector('.product-title[itemprop="name"]', { timeout: 10000 }),
        this.page.waitForSelector('h1[itemprop="name"]', { timeout: 10000 }),
      ]);
    } catch (error) {
    ...
    const productData = await this.page.evaluate((pageUrl) => {
      ...
      // Basado en la imagen 6, la marca está en el alt y title del img dentro de span.imagen-ref
      let marca = '';
      const imgBrand = document.querySelector('span.imagen-ref img');
      if (imgBrand) {
        marca = imgBrand.getAttribute('alt') || imgBrand.getAttribute('title') || '';
      }

      // Si no se encuentra con ese selector, buscar alternativas
      if (!marca) {
        // Buscar imágenes con alt o title que pueda ser la marca
        const imgElements = document.querySelectorAll('img[alt], img[title]');
        for (const img of imgElements) {
          const altText = img.getAttribute('alt');
          const titleText = img.getAttribute('title');
          
          // Verificar si el alt o title puede ser una marca de zapatillas
          if (altText && ['Nike', 'Adidas', 'Puma', 'Saucony', 'Hoka', 'New Balance'].some(brand => 
            altText.includes(brand))) {
            marca = altText;
            break;
          }
          
          if (titleText && ['Nike', 'Adidas', 'Puma', 'Saucony', 'Hoka', 'New Balance'].some(brand => 
      ...
      // Basado en la imagen 4-5, el modelo completo está en h1.h1-main con itemprop="name"
      let modeloCompleto = getText('h1.h1-main[itemprop="name"]');
      
      // Si no se encuentra, probar con otros selectores
      if (!modeloCompleto) {
        modeloCompleto = getText('h1[itemprop="name"]') || 
                         getText('.product-title[itemprop="name"]') ||
                         getText('.product_name') ||
                         getText('.h1-main.productpage_title');
      }
      ...
      // Basado en la imagen 6, el SKU está en un span con itemprep="sku"
      let sku = '';
      const skuElement = document.querySelector('span[itemprop="sku"]');
      if (skuElement) {
        sku = skuElement.textContent?.trim() || '';
        
        // Seguir las instrucciones: quitar el ":" y la talla que viene después
        if (sku.includes(':')) {
          sku = sku.split(':')[0].trim();
        }
      }
      
      // Si no encontramos el SKU con el selector anterior, buscar un elemento que contenga "Ref:"
      if (!sku) {
        const elements = document.querySelectorAll('p, div, span');
        for (const el of elements) {
          const text = el.textContent || '';
          if (text.includes('Ref:')) {
            // Formato según la imagen 6: "Ref: S60814-3:36"
            // Necesitamos extraer "S60814-3" sin la parte de la talla después del ":"
            const refMatch = text.match(/Ref:\s*([A-Za-z0-9-]+)(?::|\s|$)/i);
            if (refMatch && refMatch[1]) {
              sku = refMatch[1].trim();
              console.log(`SKU extraído de Ref: "${sku}"`);
              break;
            }
          }
        }
      ...
      // Extraer el precio
      let precioText = getText('.product-price-and-shipping') || 
      ...
      // Obtener la URL de la imagen (prioridad a figure > img como se ve en la captura)
      let imagen = '';
      
      // Primero intentar con la imagen dentro de figure (como en la captura enviada)
      const figureImgElement = document.querySelector('figure.mz-figure img, figure img[itemprop="image"]');
      if (figureImgElement) {
        imagen = figureImgElement.getAttribute('src') || '';
        console.log(`URL de imagen extraída desde figure: "${imagen}"`);
      }
      
      // Si no se encuentra, probar con otros selectores comunes
      if (!imagen) {
        const imgElement = document.querySelector('.product-image img') || 
                           document.querySelector('.product_img_link img') ||
                           document.querySelector('.product-cover img') ||
                           document.querySelector('.MagicZoom img');
        
        if (imgElement) {
          imagen = imgElement.getAttribute('src') || '';
          console.log(`URL de imagen extraída desde selectores alternativos: "${imagen}"`);
        }
      }
      
      // Si aún no hay imagen, buscar cualquier imagen relevante
      if (!imagen) {
        // Buscar cualquier imagen que tenga el modelo o marca en su alt o src
        const allImages = document.querySelectorAll('img');
        for (const img of allImages) {
          const src = img.getAttribute('src') || '';
          const alt = img.getAttribute('alt') || '';
          
          if (src.includes('-large_default') || 
              alt.includes(modeloCompleto) || 
              (marca && alt.includes(marca))) {
            imagen = src;
            console.log(`URL de imagen encontrada por contenido: "${imagen}"`);
            break;
          }
        }
      }
      ...
      console.log(`URL de imagen final: "${imagen}"`);
      ...
      const tallasElements = document.querySelectorAll('select[name="group[2]"] option, .product-variants-item select option, .product-variants option');
      ...
      if (tallas.length === 0) {
        const tallaSpans = document.querySelectorAll('.variant-option, .talla-item, .product-variants-item li, .input-container');
      ...
      if (!sku && pageUrl) {
        ...
        // La URL tiene el formato de la imagen 3:
        // thesneakerone.com/saucony/SAUCONY-PROGRID-TRIUMPH-4---GLOSSY-PINK-S60814-3?id=17595#/
        
        // Buscar la parte que contiene el código alfanumérico + guiones
        for (const parte of urlPartes) {
          // Buscar patrones como "S60814-3"
          const skuMatch = parte.match(/([A-Za-z0-9]+-[A-Za-z0-9]+)/);
          if (skuMatch && skuMatch[1]) {
            // Verificar si tiene un "?" seguido de parámetros
            const cleanSku = skuMatch[1].split('?')[0];
            sku = cleanSku;
        ...
        // Si aún no tenemos SKU, buscar en la última parte de la URL antes del "?"
        if (!sku) {
          const ultimaParte = urlPartes[urlPartes.length - 1].split('?')[0];
          if (ultimaParte.includes('-')) {
            // Extraer el código después del último "-"
            const partes = ultimaParte.split('-');
            const posibleSku = partes[partes.length - 1];
            // Verificar si parece un SKU (alfanumérico)
            if (/^[A-Za-z0-9-]+$/.test(posibleSku)) {
              sku = posibleSku;
              console.log(`SKU extraído de la última parte de URL: "${sku}"`);
            }
          }
        }
      }
